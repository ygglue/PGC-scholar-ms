  ...
  return (
    <div className="mb-6">
      ...
      <div className="space-y-3">
        {/* COR */}
        <div className={`p-5 rounded-[28px] flex items-center gap-4 transition-all ${binCOR ? 'bg-surface-container/50 opacity-60 border border-outline-variant' : 'bg-surface-container-lowest shadow-sm hover:shadow-md'}`}>
          ...
          <div className="flex-1">
            ...
            {binCOR
              ? <p className="text-xs text-on-surface-variant font-medium">{binCOR.file_name} • Uploaded {new Date(binCOR.uploaded_at).toLocaleDateString()}</p>
          ...
          <button
            ...
            className="bg-primary text-on-primary px-4 py-2 rounded-full text-xs font-bold shadow-md hover:bg-primary-container transition-colors"
            style={{ opacity: uploadStatus[`${keyPrefix}-COR`] === 'uploading' ? 0.7 : 1 }}>
            {uploadStatus[`${keyPrefix}-COR`] === 'uploading' ? 'Uploading...' : binCOR ? 'Replace' : 'Upload'}
          </button>
        </div>

        {/* ROG */}
        <div className={`p-5 rounded-[28px] flex items-center gap-4 transition-all ${binROG ? 'bg-surface-container/50 opacity-60 border border-outline-variant' : 'bg-surface-container-lowest shadow-sm hover:shadow-md'}`}>
          <div className="w-12 h-12 rounded-2xl bg-primary-fixed flex items-center justify-center text-primary shrink-0">
          ...
          <div className="flex-1">
            ...
            {binROG
              ? <p className="text-xs text-on-surface-variant font-medium">{binROG.file_name} • Uploaded {new Date(binROG.uploaded_at).toLocaleDateString()}</p>
          ...
          <button
            ...
            className="bg-primary text-on-primary px-4 py-2 rounded-full text-xs font-bold shadow-md hover:bg-primary-container transition-colors"
            style={{ opacity: uploadStatus[`${keyPrefix}-ROG`] === 'uploading' ? 0.7 : 1 }}>
            {uploadStatus[`${keyPrefix}-ROG`] === 'uploading' ? 'Uploading...' : binROG ? 'Replace' : 'Upload'}
          </button>
        </div>

        {/* Letter */}
        <div className={`border-2 border-dashed border-outline-variant p-5 rounded-[28px] flex items-center gap-4 transition-all ${binLetter ? 'bg-surface-container/30 opacity-60' : 'bg-surface-container/50'}`}>
          ...
          <div className="flex-1">
            ...
            {binLetter
              ? <p className="text-xs text-on-surface-variant font-medium">{binLetter.file_name} • Uploaded {new Date(binLetter.uploaded_at).toLocaleDateString()}</p>
              : <p className="text-xs text-on-surface-variant">Update the board on your progress</p>}
          </div>
          <button
            onClick={() => triggerUpload('explanation_letter', bin.id, binLetter?.id)}
            disabled={uploadStatus[`${keyPrefix}-explanation_letter`] === 'uploading'}
            className="text-primary hover:bg-primary-fixed px-4 py-2 rounded-full text-xs font-bold transition-colors"
            style={{ opacity: uploadStatus[`${keyPrefix}-explanation_letter`] === 'uploading' ? 0.7 : 1 }}>
            {uploadStatus[`${keyPrefix}-explanation_letter`] === 'uploading' ? 'Uploading...' : binLetter ? 'Replace' : 'Upload'}
          </button>
        </div>
      </div>
    </div>
...
const Dashboard = () => {
  ...
  const { data: bins } = useApiCache('submission_bins', `${API_BASE}/submission-bins/`);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (oldDocId) {
        try { await apiClient.delete(`${API_BASE}/documents/${oldDocId}`); }
        catch (delErr) { console.warn('Failed to delete old document:', delErr); }
      }
  ...
  if (!profile || !documents || !bins) return (
    <Layout>
  ...
  return (
    <Layout>
      <input type="file" ref={fileInputRef} onChange={handleFileUpload} accept=".jpg,.jpeg,.png,.pdf" className="hidden" />
      ...
      <section className="mb-10">
        <h4 className="text-sm font-bold uppercase tracking-wide text-on-surface-variant mb-4">Required Submissions</h4>

        {activeBins.length === 0 ? (
          <div className="bg-surface-container/50 border-2 border-dashed border-outline-variant rounded-[28px] p-8 text-center">
            <span className="material-symbols-outlined text-on-surface-variant text-4xl mb-2 block">inbox</span>
            <p className="text-sm font-bold text-on-surface">No Submission Bins yet</p>
            <p className="text-xs text-on-surface-variant mt-1">Your evaluator hasn't opened a submission window.</p>
          </div>
        ) : (
          activeBins.map(bin => (
            <BinUploadSection
              key={bin.id}
              bin={bin}
              binDocs={(documents || []).filter(d => d.submission_bin_id === bin.id)}
      ...
      <section className="mb-12">
        ...
        {sortedBinGroups.length === 0 ? (
          <div className="p-4 text-center text-on-surface-variant text-sm">No submissions yet</div>
        ) : (
          <div className="space-y-3">
            {sortedBinGroups.map(([binId, { label, docs }]) => (
              <details key={binId} className="group/accordion bg-surface-container-lowest rounded-[24px] overflow-hidden shadow-sm border border-surface-container-low">
                <summary className="w-full flex items-center justify-between p-4 hover:bg-surface-container-high/30 transition-colors cursor-pointer select-none">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-[10px] text-on-surface-variant uppercase tracking-wider">{docs.length} FILE{docs.length !== 1 ? 'S' : ''}</p>
                    </div>
                  </div>
                  <span className="material-symbols-outlined text-on-surface-variant transition-transform duration-300">expand_more</span>
                </summary>

                <div className="bg-surface-container-low/20 px-2 pb-2 pt-1 space-y-1">
                  {docs.map(doc => (
                    <div key={doc.id} className="flex items-center justify-between p-3 rounded-2xl hover:bg-white transition-colors cursor-pointer group/file" onClick={() => handleViewDocument(doc.id)}>
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-surface-container flex items-center justify-center">
                          <span className="material-symbols-outlined text-on-surface-variant text-base">
                            {doc.doc_type === 'COR' ? 'description' : doc.doc_type === 'ROG' ? 'grade' : 'mail'}
                          </span>
                        </div>
                        <div>
                      ...
                      <div className="flex items-center gap-3">
                        <span className={`text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${doc.is_verified ? 'bg-primary-fixed text-on-primary-fixed' : 'bg-surface-container-highest text-on-surface-variant'}`}>
