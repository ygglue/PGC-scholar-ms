...
## Corpus Check
- 90 files · ~83,116 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 472 nodes · 896 edges · 49 communities detected
...
Cohesion: 0.05
Nodes (31): createAnnouncement(), editAnnouncement(), getUserIdFromToken(), initUser(), loadAnnouncements(), handleView(), loadDocuments(), CacheService (+23 more)
...
Cohesion: 0.09
Nodes (12): QWidget, create_ambient_shadow(), FetchScholarDocsThread, FetchScholarsThread, get_avatar_color(), get_initials(), PaginationControl, Generate initials: Juan + Dela Cruz -> "JDC" (+4 more)
...
- **Thin community `Community 25`** (2 nodes): `test_supabase_direct.py`, `test_upload()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 26`** (2 nodes): `main.py`, `health_check()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 37`** (1 nodes): `vite-env.d.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 38`** (1 nodes): `Modal.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `Extract token from Authorization header or cookie.` connect `Community 1` to `Community 0`?**
...
- **Are the 17 inferred relationships involving `PendingChange` (e.g. with `Updates the last_updated_at timestamp in the system_sync table      for the spe` and `GradeInput`) actually correct?**
  _`PendingChange` has 17 INFERRED edges - model-reasoned connections that need verification._