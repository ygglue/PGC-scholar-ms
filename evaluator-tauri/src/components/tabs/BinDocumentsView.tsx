...
interface BinDocumentsViewProps {
  bin: BinData;
}

export const BinDocumentsView: React.FC<BinDocumentsViewProps> = ({ bin }) => {
  ...
  return (
    <div className="p-8 h-full flex flex-col">
      <button onClick={handleBack} className="flex items-center gap-2 text-sm text-[#4A5568] dark:text-dark-text-sec hover:text-[#1A8C3C] mb-6 shrink-0 transition-colors">
          ...
          {Object.entries(grouped).map(([scholarId, scholarDocs]) => (
            <div key={scholarId} className="bg-white dark:bg-dark-card p-6 rounded-2xl border border-[#E0E6E0] dark:border-dark-border shadow-sm">
              <div className="flex items-center gap-3 mb-5 pb-4 border-b border-[#F0F2F0] dark:border-dark-border">
                <div className="w-10 h-10 rounded-full bg-[#E8F5ED] dark:bg-dark-green-bg flex items-center justify-center text-[#1A8C3C] dark:text-dark-green font-bold text-sm">
                ...
                {scholarDocs.map(doc => (
                  <div key={doc.id} className="bg-[#F7F9F7] dark:bg-dark-surface p-4 rounded-xl flex items-center justify-between border border-[#E0E6E0]/50 dark:border-dark-border/50 hover:border-[#1A8C3C]/30 transition-colors">
                    ...
                    <div className="flex items-center gap-4">
                      <span className={`text-[10px] font-bold px-3 py-1 rounded-full ${doc.is_verified ? 'bg-[#E8F5ED] text-[#1A8C3C] dark:bg-dark-green-badge dark:text-dark-green' : 'bg-gray-100 dark:bg-dark-surface text-gray-600 dark:text-dark-text-sec'}`}>
